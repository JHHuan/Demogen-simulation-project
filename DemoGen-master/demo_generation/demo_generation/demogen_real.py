...
from demo_generation.real_mask_util import get_objects_pcd_from_sam_mask_real
import imageio
...
try:
    ...
    O3D_AVAILABLE = False
    cprint("Warning: open3d not available, outlier removal will be skipped", "yellow")
...
class DemoGen:
    def __init__(self, cfg):
        ...
        # Only load target_trans_range for two-stage tasks
        self.target_trans_range = cfg.trans_range[self.gen_name].get("target", [[0,0,0],[0,0,0]])
    ...
    def get_objects_pcd_from_sam_mask(self, pcd, demo_idx, object_or_target="object"):
        """
        真实环境版本：使用SAM mask提取物体点云

        真实环境mask文件结构：
        sam_mask/{source_name}/{demo_idx}/obj1.jpg  (物体1)
        sam_mask/{source_name}/{demo_idx}/obj2.jpg  (物体2/目标)
        """
        assert object_or_target in ["object", "target"]

        # 真实环境使用obj1和obj2作为mask文件名
        if object_or_target == "object":
            mask_name = "obj1"
        else:  # target
            mask_name = "obj2"

        mask_path = os.path.join(self.data_root, f"sam_mask/{self.source_name}/{demo_idx}/{mask_name}.jpg")
        ...
        # 真实环境的mask可能来自不同分辨率的图像
        # 使用mask的原始shape而不是固定的高分辨率shape
        depth_shape = mask.shape[:2]  # (height, width)
        filtered_pcd = get_objects_pcd_from_sam_mask_real(pcd, mask, depth_shape)
    ...
    def two_stage_augment(self, n_demos, render_video=False, gen_mode='random'):
        ...
        for i in range(self.n_source_episodes):
            ...
            if self.use_manual_parsing_frames:
                ...
                ee_poses = source_demo["agent_pos"][:, :3]
            ...
            for trans_vec in tqdm(trans_vectors):
                ...
                for j in range(skill_1_frame):
                    if not self.use_linear_interpolation:
                        current_step_delta = step_actions[j]
                    else:
                        current_step_delta = step_action  # 每一帧的增量（速度）

                    # 【关键修复】累加计算绝对位置
                    curr_interp_pos += current_step_delta

                    # 获取原始动作（用于保持夹爪状态）
                    source_action = source_demo["action"][current_frame]

                    # 【关键修复】保存绝对位置（不是增量）
                    new_action = np.concatenate([curr_interp_pos, source_action[3:]], axis=0)
                    traj_actions.append(new_action)

                    # 【关键修复】计算trans_sofar = 目标位置 - 原始位置
                    source_pos_at_frame = source_action[:3]
                    trans_sofar = curr_interp_pos - source_pos_at_frame
                ...
                for k in range(skill_2_frame - motion_2_frame):
                    if not self.use_linear_interpolation:
                        current_step_delta = step_actions[k]
                    else:
                        current_step_delta = step_action  # 每一帧的增量（速度）
    ...
    def one_stage_augment(self, n_demos, render_video=False, gen_mode='random'):
        ...
        for i in tqdm(range(self.n_source_episodes)):
            ...
            for obj_trans_vec in tqdm(trans_vectors):
                ...
                while current_frame < num_frames:
                    action = source_demo["action"][current_frame].copy()
                    # 【关键修复】skill阶段的action也需要平移，使其与平移后的物体位置对齐
                    # 仿真数据中，action[:3]是绝对位置
                    action[:3] += trans_sofar
                    traj_actions.append(action)
    ...
    @staticmethod
    def pcd_divide(pcd, bbox_list, enable_outlier_removal=True):
        """
        pcd: (n, 6)
        bbox_list: list of (2, 3), [[x_min, y_min, z_min], [x_max, y_max, z_max]]
        enable_outlier_removal: 对机械臂点云进行离群点过滤
        :return: list of pcds
        """
        masks = []
        selected_pcds = []
        ...
        # 对机械臂点云（最后一个）进行离群点过滤（暂时禁用）
        # if enable_outlier_removal and len(selected_pcds) > 0:
        #     robot_pcd = selected_pcds[-1]
        #     if len(robot_pcd) > 50:  # 只有点数足够时才过滤
        #         robot_pcd = DemoGen.statistical_outlier_removal(robot_pcd, nb_neighbors=20, std_ratio=2.0)
        #         selected_pcds[-1] = robot_pcd
    ...
    def statistical_outlier_removal(pcd, nb_neighbors=20, std_ratio=2.0):
        """
        使用统计滤波去除离群点（不依赖open3d，使用scipy实现）

        Args:
            pcd: (N, 6) 点云 [XYZ, RGB]
            nb_neighbors: 计算平均距离时考虑的邻居数量
            std_ratio: 标准差倍数阈值
        ...
        # 保留距离在 mean - std_ratio * std 范围内的点
    ...
    def translate_all_frames(source_demo, trans_vec, start_frame=0):
        ...
        """
        source_states = source_demo["agent_pos"]
        ...
        states[:, :3] += trans_vec
        # 【关键修复】action也需要平移（仿真数据中action[:3]是绝对位置）
        actions = source_actions[start_frame:].copy()
        actions[:, :3] += trans_vec
