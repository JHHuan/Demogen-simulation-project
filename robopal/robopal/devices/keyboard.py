...
class Keyboard(BaseDevice):
    ...
    def command_introduction(self):
        ...
        logging.info("Press <SPACE> to manually record one frame.")
    ...
    def on_press(self, key):
        try:
            if key == keyboard.Key.up:
                ...
                logging.info(f"[DEBUG on_press] 检测到空格键按下！")
    ...
    def on_release(self, key):
        ...
        if key == keyboard.Key.ctrl_l:
            ...
            logging.info(f"[DEBUG on_release] 空格键释放（不做任何处理）")
            # 空格键只在on_press中处理，这里不重复设置标志
