...
物体: assembly (MetaWorld assembly task)
  - round_nut: 扳手/圆环, 有 freejoint, 可被抓取移动
  - peg: 圆柱销, 静态固定在桌面上, 需要将圆环套入
...
from robopal.robots.panda import PandaAssembly3cam
...
GRIPPER_FORCE_RANGE = 220.0
GRIPPER_SERVO_SCALE = 3.0
FINGERTIP_FRICTION = np.array([3.0, 0.08, 0.02], dtype=np.float64)
CONTACT_SOLREF = np.array([0.008, 1.0], dtype=np.float64)
GRIPPER_SETTLE_STEPS = 60
Z_KEY_ROTATION_STEP = np.pi / 4


def _get_mj_id(model, obj_type, name):
    obj_id = mujoco.mj_name2id(model, obj_type, name)
    return None if obj_id == -1 else obj_id


def _set_body_contact_params(model, body_name, *, friction=None, condim=None, solref=None):
    body_id = _get_mj_id(model, mujoco.mjtObj.mjOBJ_BODY, body_name)
    if body_id is None:
        logging.warning(f"未找到 body: {body_name}")
        return 0

    updated = 0
    for geom_id, geom_body_id in enumerate(model.geom_bodyid):
        if geom_body_id != body_id or model.geom_contype[geom_id] == 0:
            continue
...
def apply_assembly_grasp_tuning(env):
    """增强 assembly 任务里的夹爪力和接触摩擦，尽量不改变原始开合范围。"""
    actuator_id = _get_mj_id(env.mj_model, mujoco.mjtObj.mjOBJ_ACTUATOR, '0_actuator8')
    if actuator_id is not None:
        env.mj_model.actuator_forcerange[actuator_id] = np.array(
            [-GRIPPER_FORCE_RANGE, GRIPPER_FORCE_RANGE], dtype=np.float64
        )
        env.mj_model.actuator_gainprm[actuator_id, 0] *= GRIPPER_SERVO_SCALE
        env.mj_model.actuator_biasprm[actuator_id, 1] *= GRIPPER_SERVO_SCALE
        env.mj_model.actuator_biasprm[actuator_id, 2] *= GRIPPER_SERVO_SCALE
    else:
        logging.warning("未找到夹爪执行器 0_actuator8，跳过夹爪力度调参")

    left_count = _set_body_contact_params(
        env.mj_model,
        '0_left_finger',
        friction=FINGERTIP_FRICTION,
        condim=4,
        solref=CONTACT_SOLREF,
    )
    right_count = _set_body_contact_params(
        env.mj_model,
        '0_right_finger',
        friction=FINGERTIP_FRICTION,
        condim=4,
    ...
    logging.info(
        "已应用抓取增强参数: actuator8 force=%.1f servo x%.1f, finger geoms=%d",
        GRIPPER_FORCE_RANGE,
        GRIPPER_SERVO_SCALE,
...
class AssemblyKeyboard(Keyboard):
    """给 assembly 遥操作增加末端绕工具轴旋转快捷键。"""

    def __init__(self, pos_scale=0.01, rot_scale=0.01, gripper_rot_scale=None) -> None:
        super().__init__(pos_scale=pos_scale, rot_scale=rot_scale)
        self.gripper_rot_scale = rot_scale if gripper_rot_scale is None else gripper_rot_scale

    def command_introduction(self):
        super().command_introduction()
        logging.info("Press <Z> to rotate the gripper by 45 deg around the tool axis, <X> for fine reverse rotation.\n")
        ...
        try:
            ...
            if char == 'z':
                self._end_rot_offset = self._end_rot_offset.dot(
                    T.euler_2_mat(Z_KEY_ROTATION_STEP * np.array([0, 0, 1]))
                )
            elif char == 'x':
                self._end_rot_offset = self._end_rot_offset.dot(
                    T.euler_2_mat(self.gripper_rot_scale * np.array([0, 0, -1]))
                )
...
def main():
    ...
    keyboard = AssemblyKeyboard(pos_scale=0.01, rot_scale=0.01)
    keyboard.start()
    env.reset()
    apply_assembly_grasp_tuning(env)
